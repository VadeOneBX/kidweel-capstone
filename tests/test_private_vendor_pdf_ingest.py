"""Tests for private vendor PDF ingest path validation and extraction."""
...
def test_private_path_constants() -> None:
    assert PRIVATE_RAW_ROOT == Path("private/raw")
    assert PRIVATE_TEXT_ROOT == Path("private/text")
    assert PRIVATE_PARSED_ROOT == Path("private/parsed")


def test_raw_pdf_path_must_be_under_private_raw(tmp_path: Path) -> None:
    bad = tmp_path / "data" / "macro_note.pdf"
    bad.parent.mkdir(parents=True)
    bad.write_bytes(b"%PDF-1.4")
    with pytest.raises(ValueError, match="private/raw"):
        assert_private_raw_pdf_path(bad, base_dir=tmp_path)

    good = tmp_path / "private" / "raw" / "macro_note_2026_07_09.pdf"
...
def test_output_path_must_be_under_private_parsed(tmp_path: Path) -> None:
    bad = tmp_path / "data" / "parsed" / "macro_note.json"
    with pytest.raises(ValueError, match="private/parsed"):
        assert_private_output_path(bad, base_dir=tmp_path)

    good = tmp_path / "private" / "parsed" / "macro_note_2026_07_09.json"
...
def test_extract_pdf_text_from_synthetic_pdf(tmp_path: Path) -> None:
    ...
    pdf_path = tmp_path / "private" / "raw" / "macro_note_2026_07_09.pdf"
...
def test_extract_pdf_text_empty_returns_needs_review(tmp_path: Path) -> None:
    ...
    pdf_path = tmp_path / "private" / "raw" / "empty.pdf"
...
def test_write_parsed_json_marks_proprietary(tmp_path: Path) -> None:
    out = tmp_path / "private" / "parsed" / "macro_note_2026_07_09.json"
